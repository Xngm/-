...
static void drawImage(int x, int y,const IMAGE *src)
{


	// 变量初始化
	DWORD* pwin = GetImageBuffer();			//窗口缓冲区指针
	const DWORD* psrc = GetImageBuffer(const_cast<IMAGE*>(src));		//图片缓冲区指针
...
static void drawImage(int x, int y, int dstW, int dstH, IMAGE* src, int srcX, int srcY)
{
	if (!src)
		assert(src != NULL);
	// 变量初始化
...
static void drawPngImage(int x, int y, IMAGE* srcimg) //新版png
{
...
static void transparentImage(int x, int y, IMAGE* srcimg, UINT transparentcolor)
{
	HDC dstDC = GetImageHDC(NULL);
	HDC srcDC = GetImageHDC(srcimg);
	int w = srcimg->getwidth();
